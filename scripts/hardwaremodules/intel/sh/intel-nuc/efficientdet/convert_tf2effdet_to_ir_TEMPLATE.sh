...
get_model_name()
{
  ...
  MODELNAME=`echo $MYFILENAME | sed 's/convert_tf2effdet_to_ir_//' | sed 's/.sh//'`
...
convert_to_ir()
{
  ...
  mkdir -p exported-models-openvino/$MODELNAME\_OV$PRECISION
  
  python3 $OPENVINOINSTALLDIR/deployment_tools/model_optimizer/mo_tf.py \
  --input_model exported-models/$MODELNAME/saved_model/saved_model_frozen.pb \
  --transformations_config $OPENVINOINSTALLDIR/deployment_tools/model_optimizer/extensions/front/tf/automl_efficientdet.json \
  --input_shape [1,$width,$height,3] \
  --output_dir=exported-models-openvino/$MODELNAME\_OV$PRECISION \
  --data_type $PRECISION
  
  #--transformations_config ./automl_efficientdet_d0_pets.json \
