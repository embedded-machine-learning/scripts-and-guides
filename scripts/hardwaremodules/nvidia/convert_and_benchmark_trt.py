...
def batch_input(batch_size, data_path, precision, hw, is_keras=False):
    if precision == 'FP32':
        datatype = np.float32
    elif precision == 'FP16':
        datatype = np.float16
        ...
        raise ValueError("No valid precision provided: " + precision + ". It has to be FLOAT32, FLOAT16 or INT8")
    ...
    for i in range(batch_size):
        img_path = data_path + pics[i % 4] #generating batches
...
def predict_and_benchmark_throughput(batched_input, infer, args, N_warmup_run=50, N_run=1000):
    ...
    for i in range(N_warmup_run):
        ...
        # preds = labeling['predictions'].numpy() #for models loaded from tf.keras.applications
        preds = labeling
...
def convert_to_trt_grapch_and_save(precision_mode, input_saved_model_dir, calibration_data, output_dir=None):
    ...
    if output_dir is None:
        ...
        output_saved_model_dir = output_dir#input_saved_model_dir + converted_saved__suffix
...
def main():
    ...
    batched_input = batch_input(args.batch_size, args.data_dir, args.precision, images_size, is_keras=False)
    print("=== Batch input prepared ===")

    #conversion
    if (args.conversion):
        print("=== Convert input model to trt. Model={}, Precision={} ===".format(args.tensorflow_model, args.precision))
        convert_to_trt_grapch_and_save(args.precision, args.tensorflow_model, batched_input, args.net)
