        ...
        with open(os.path.join(annotation_filepath), 'r') as file:
            ...
            for line in lines:
                ...
                new_row = {'filename': image_file_name,
                           'width': w,
                           'height': h,
                           'class': str(int(data[0]) + 1), #TF Detections classes always start by 1 and not by 0 as Yolo
                           'xmin': xmin,
                           'ymin': ymin,
                           'xmax': xmax,
                           'ymax': ymax,
...
def convert_yolo_to_tfcsv(annotation_dir, image_dir, output_path):
    ...
    detections_df = pd.DataFrame(columns=['filename', 'width', 'height', 'class', 'xmin', 'ymin', 'xmax', 'ymax', 'score'])
